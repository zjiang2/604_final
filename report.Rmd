...
Vaccine hesitancy is a major impediment to the public health response to COVID-19. Mistrust of the COVID-19 vaccine can originate from many complex sources that can be political, historical, racial, etc. In this study, we examine the relationship between vaccination rates in US counties and demographic data. By performing a thorough quantitative study of this phenomenon, we hope to identify key factors that lead to vaccine hesitancy. Identifying these factors can be useful in crafting public health messaging to promote vaccination.
...
```{r hesitancyOverview, results="hide", fig.width=6, fig.height=3, fig.cap="Map and Distribution of 18-Plus vaccination Rates by County, June 19th, 2021"}
pp1 <- dall %>% 
  ggplot(aes(x=long, y=lat, group = group)) +
  geom_polygon(aes(fill=vax_rate_18_plus), color=NA) +
  scale_fill_gradientn(colors=c("red","orange","green","blue"), na.value="grey") +
  labs(fill = "Vax rate %") + 
  theme(text = element_text(size=5), axis.title.x = element_blank(),
...
cci_rf <- readRDS("data/cci.rds")

train_rf <- cci_rf$train[,-c(1:3,5:9,11)] %>% drop_na() %>%
  mutate(y = factor(Series_Complete_18PlusPop_Pct >= 44)) %>%
  select(-Series_Complete_18PlusPop_Pct)

rf <- randomForest(y~.,data=train_rf, ntree=200, importance=TRUE)

var_imp <- importance(rf, type=1) %>% as.data.frame() %>%
  rownames_to_column(var="Variable") %>%
  mutate(Variable = fct_reorder(Variable, MeanDecreaseAccuracy, .desc=TRUE))

rf_pp1 <- var_imp %>% ggplot(aes(x=Variable, y=MeanDecreaseAccuracy)) +
...
# Some sort of post-hoc analysis
