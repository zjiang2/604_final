...
Table 2 compares accuracy of the three models on the same test data set. The random forest and logistic regression models are virtually tied in terms of performance on unseen data, both edging out AdaBoost, which suffered from overfit during training and trained on only a subset of predictors. With careful feature selection and validation, a parametric model such as logistic regression achieves even a slightly higher accuracy than a black-box algorithm such as a random forest.
