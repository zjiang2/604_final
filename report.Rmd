...
In our post-hoc analysis, we examined the random forest model. In particular, we are interested in identifying geographic patterns in the model's classification error. To allow ourselves as complete a map of the US as possible, we decided to use the entire data set in this analysis. For a given observation $x_i$ in the training set, we predicted it using only trees not trained on the observation. Of course, while the results of the training set will not reflect the true capabilities of the random forest, we believe it will be close enough.

```{r, fig.cap="Map of counties color coded by actual classification (top), predicted classification (bottom).", fig.height=8}
# get rf predictions
train_pred_prob <- predict(rf, type='prob')[,2] %>% as.numeric()
test_pred_prob <- predict(rf, newdata=cci_rf$test[,-c(1,47)], type='prob')[,2] %>% as.numeric()
valid_pred_prob <- predict(rf, newdata=cci_rf$validate[,-c(1,47)], type='prob')[,2] %>% as.numeric()

# combine predictions, actual, FIPS
res_prob <- cbind(c(cci_rf$train$FIPS, cci_rf$test$FIPS, cci_rf$valid$FIPS),
             c(cci_rf$train$y, cci_rf$test$y, cci_rf$valid$y),
             c(train_pred_prob, test_pred_prob, valid_pred_prob)) %>% as.data.frame()
colnames(res_prob) <- c("FIPS","actual","pred")
res_prob <- res_prob %>%
  mutate(severity = ifelse((actual == 1) & (pred < 0.5),
                   abs(pred - 0.5),
                   0)) %>%
  mutate(severity = ifelse((actual == 0) & (pred > 0.5),
                   abs(pred - 0.5),
                   severity)) %>%
...
# plot
rf_ph1_new <- dall %>%
  left_join(res_prob, by=c("fips"="FIPS")) %>%
  ggplot(aes(x=long, y=lat, group = group)) +
  geom_polygon(aes(fill=Actual)) +
...
rf_ph2_new <- dall %>%
  left_join(res_prob, by=c("fips"="FIPS")) %>%
  ggplot(aes(x=long, y=lat, group = group)) +
  geom_polygon(aes(fill=Prediction), color=NA) +
  scale_fill_viridis() +
  map_theme
...
rf_ph3_new <- dall %>%
  left_join(res_prob, by=c("fips"="FIPS")) %>%
  ggplot(aes(x=long, y=lat, group = group)) +
  geom_polygon(aes(fill=Severity), color=NA) +
  scale_fill_viridis(option = 'B') +
  map_theme

ggarrange(rf_ph1_new, rf_ph2_new,
          nrow=2,
          ncol=1) +
  ggtitle('Comparison of Actual vs. Predicted Values')
```
We plot the actual label for each county (one for vaccine-accepting, zero for vaccine-hesitant) along with a map of our predicted probabilities for each county being vaccine-accepting (a number ranging from zero to one). We can see visually that our predictions match reality quite well. The South and mid-Atlantic regions are heavily purple in our predictions, indicative of a high level of confidence that these counties are vaccine-hesitant. Our classifier predicts vaccine-accepting counties most heavily in the Northeast/New England region as well as in California, Florida, and the upper Midwest. All of these trends match reality. 

To examine where our model is indecisive, we can examine portions of the map that look green/turquoise and which have probabilities that hover around 50%. We see lots of uncertainty in the middle of the country, primarily in the Kansas and Nebraska area, which appears reasonable given the sharp shift from vaccine-acceptance to hesitancy that we see in the map of ground truth. Our model also appears uncertain with dealing with western states with lower population levels like Wyoming, Utah, Nevada, Montana, and Colorado, a fact that concurs with the large amount of adjacent label switching that occurs in these regions of the country. 

Overall, our predictions are rather accurate and match reality with significant detail. For example, the predictions for the upper region of lower peninsula Michigan match extremely well with the ground truth, as do our predictions for counties in the state of Florida.
...
In table 4, we looked at the misclassifications on a county level. In particular, we looked at the counties that had the worst misclassifications - highly vaccinated counties that were classified vaccine hesitant and vice versa. We see that random forest gets false positives in counties that are wealthy and/or supported Joe Biden in the 2020 Presidential election. In our data of counties, the median county income per capita is 23576, and the third quartile is 27066. Counties that get wrongfully classified as vaccine hesitant have characteristics of being conservative and/or less wealthy and/or largely Black. In generally, counties that get misclassified buck the trend normally associated with vaccine acceptance.
...
\hline
Worst FN & Martin, NC & 0.859 & 0.471 & 19032 & 0.435\\
2nd Worst FN & Santa Cruz, AZ & 0.854 & 0.672 & 17795 & 0.002\\
...
```{r, fig.cap="Out-of-bag Error Rate for Random Forest Classifier on April data set.",fig.height=3,fig.width=5}
# Rerun random forest
# get oob error rates for various number of trees
