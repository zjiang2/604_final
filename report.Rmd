...
As of December 2021, about 59% of the US population has been fully vaccinated against COVID-19. Despite America's wealth, advanced medical sector, and its creation of the first vaccine, America ranks just 19th in the world in terms of vaccination rates.[^1] Given the abundance of domestic vaccine doses, it is reasonable to assume that large segments of the American populace are unwilling to take the vaccine. Media narratives attribute this public hesitancy to many causes, ranging from ranging from political polarization to consumer anxiety about hidden costs.
...
We explored whether or not a parametric model could accurately explain our data using logistic regression. This model assumes that the log odds of the probability that a given county is vaccine-hesitant can be written as a linear combination of our predictor variables, i.e. \begin{equation}
...
AdaBoost            & .829          \\ \hline
Random Forest       & .851          \\ \hline
Logistic Regression & .853          \\ \hline
\end{tabular}
\end{table}
```
The random forest and logistic regression models are virtually tied in terms of performance on unseen data, both edging out AdaBoost, which suffered from overfit during training. With careful feature selection and validation, a parametric model such as logistic regression achieves even a slightly higher accuracy than a black-box algorithm such as a random forest.
...
There appears to be a concentration of misclassifications at the border of hesitancy regions. For example, there are many false negatives in parts of Illinois, Indiana, Ohio, Kentucky. Those states are known to be the boundary between the Midwest/Northeast, which are generally vaccine willing regions, and the South, which contains many of the most vaccine hesistant areas.
...
```{r, fig.cap="Feature importance for random forest classifier.", fig.height=8}
# run rf using optimal number of trees
...
[concluding remarks. could regression have worked? how to craft effective health messaging in a climate of skepticism toward institutions.]
# Bibliography
